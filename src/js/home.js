...
function generateHiddenSEOContent() {
    ...
    const generalSEOText = currentLang === 'en' ? 
        ...
        '电子元器件代理商 村田 国巨 TDK 三星 太阳诱电 太诱 爱普生 厚声 授权经销商 正品元件 现货库存 快速发货 全球服务 贴片电容 MLCC 多层陶瓷电容器 太诱代理商 太阳诱电代理商 Taiyo Yuden distributor';
    
    // 添加专门的太诱和贴片电容SEO内容
    const taiyoYudenSEOText = currentLang === 'en' ? 
        'Taiyo Yuden authorized distributor MLCC ceramic capacitors chip capacitors Taiyo Yuden distributor authentic components stock inventory fast delivery technical support' :
        '太阳诱电授权代理商 太诱代理商 贴片电容 MLCC 多层陶瓷电容器 太诱电容 原装正品 现货库存 快速交付 技术支持 电容器代理 太阳诱电分销商 Taiyo Yuden distributor';
    
    seoContainer.innerHTML = `
        <h2>${currentLang === 'en' ? 'Hot Selling Electronic Components Models' : '热卖电子元器件型号'}</h2>
        <p>${seoText}</p>
        <p>${generalSEOText}</p>
        <h3>${currentLang === 'en' ? 'Taiyo Yuden MLCC Capacitors Distributor' : '太阳诱电太诱贴片电容代理商'}</h3>
        <p>${taiyoYudenSEOText}</p>
    `;
}
